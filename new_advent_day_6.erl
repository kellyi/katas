-module(new_advent_day_6).
-export([solve/0]).

parse({ok, F}) -> string:tokens(binary:bin_to_list(F), "\n").
format(L) -> lists:map(fun(X) -> most_common_char(X) end, L).
...
most_common_char(List) ->
    Char = lists:nth(1, List),
    Count = count_char(List, Char),
    most_common_char(List, List, Char, Count).
most_common_char([], _, Char, _) -> Char;
most_common_char([H|T], List, Char, Count) ->
    NewCount = count_char(List, H),
    case NewCount > Count of 
        true -> most_common_char(T, List, H, NewCount);
        _ -> most_common_char(T, List, Char, Count)
    end.
